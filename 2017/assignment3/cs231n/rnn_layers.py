...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        ...
        curr_h, cache_curr = rnn_step_forward(x[:,i,:],curr_h,Wx,Wh,b)
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        ...
        dx[:, i, :], dprev_h, dWx1, dWh1, db1 = rnn_step_backward(dh_curr, cache[i])
        dWx+=dWx1
        dWh+=dWh1
        db+=db1
    dh0 = dprev_h    
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dtanh = dtanh * (1 - np.square(np.tanh(next_c)))
    dc = dtanh+ dnext_c
    df = dc*prev_c
    dprev_c = dc*f
    di = dc*g
...
def lstm_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        dh_curr = dprev_h + dh[:,i,:]
        dx[:, i, :], dprev_h,dprev_c, dWx1, dWh1, db1 = lstm_step_backward(dh_curr,dprev_c,cache[i])
        dWx+=dWx1
        dWh+=dWh1
        db+=db1
    dh0 = dprev_h   
