...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        capt_vectors,cache_embed = word_embedding_forward(captions_in, W_embed)
        if self.cell_type == 'rnn':
            h_rnn,cache_rnn = rnn_forward(capt_vectors, h_init, Wx, Wh, b)
        elif self.cell_type == 'lstm':
            h_rnn,cache_rnn = lstm_forward(capt_vectors, h_init, Wx, Wh, b)    
        scores,cache_scores = temporal_affine_forward(h_rnn, W_vocab, b_vocab) 
        loss, dscores = temporal_softmax_loss(scores, captions_out, mask)
        
        #backward pass
        
        dscores,grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dscores, cache_scores)
        if self.cell_type == 'rnn':
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
            if self.cell_type == 'rnn':
                ...
                h,c, _ = lstm_step_forward(word_embed,c,h,Wx,Wh,b)    
