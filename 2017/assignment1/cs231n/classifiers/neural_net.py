...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    correct_class_score = np.choose(y,normalised_scores.T)
    loss = -correct_class_score + np.log(np.sum(np.exp(normalised_scores),axis=1))
    loss = np.sum(loss)
    loss/=N
    loss += reg*(np.sum(W1*W1) + np.sum(b1*b1) + np.sum(W2*W2) + np.sum(b1*b1))
